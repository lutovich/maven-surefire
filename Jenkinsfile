...
def cmd = ['mvn']
def options = ['-B', '-U', '-e', '-V', '-P', 'run-its']
def goals = ['clean', 'install', 'jacoco:report']
def int port = env.IT_PORT ? env.IT_PORT : 8084
def Map stages = [:]
...
oses.eachWithIndex { os, indexOfOs ->
    ...
    mavens.eachWithIndex { maven, indexOfMaven ->
        ...
        jdks.eachWithIndex { jdk, indexOfJdk ->
            ...
            def integrationTestPort = port++
            println "integrationTestPort=${integrationTestPort}"
                            ...
                            dir('build') {
                                ...
                                stage("build ${stageKey}") {
                                    ...
                                    withMaven(jdk: 'JDK 1.8 (latest)', maven: mvnName, mavenLocalRepo: mvnLocalRepoDir,
                                        ...
                                        def properties = ["\"-Djdk.home=${jdkHome}\"", "-Dintegration-test-port=${integrationTestPort}", "-Djacoco.skip=true"]
                                        def command = cmd + options + goals + properties
                                        bat command.join(' ')
                                    }
                                }
                            }
                            ...
                            dir('build') {
                                ...
                                stage("build ${stageKey}") {
                                    ...
                                    withMaven(jdk: 'JDK 1.8 (latest)', maven: mvnName, mavenLocalRepo: mvnLocalRepoDir,
                                        ...
                                        def properties = ["\"-Djdk.home=${jdkHome}\"", "-Dintegration-test-port=${integrationTestPort}", "-Djacoco.skip=${skipJacoco}"]
                                        def command = cmd + options + goals + properties
                                        sh command.join(' ')
                                    }
                                }
